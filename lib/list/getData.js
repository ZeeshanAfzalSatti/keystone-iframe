		...
		fields.forEach(function (path) {
			...
			if (field) {
				if (field.type === 'relationship' && expandRelationshipFields) {
					...
					const fieldVal = field.getData(item)
					if (!isNaN(fieldVal) && _.includes(_.toString(fieldVal), '.')) {
						data.fields[path] = _.toNumber(fieldVal).toFixed(2);
					} else {

						data.fields[path] = fieldVal
					}
				}
			} else {

...
function transformFieldValue(field, item, options) {
	...
	const result = item[field.path]
	if (_.isObject(result)) {
		return moment(result).isValid() && _.size(result) === 0 ? moment(result).format('MMMM Do YYYY, h:mm:ss a') : result.name
	}
	...
	if (options.fields) {
		...
		options.fields.forEach(function (path) {
			...
			if (!field) {
				...
				data[changeCase.upperCase(changeCase.sentenceCase(path))] = item.get(path);
				return;
			}
			if (field.type !== 'relationship' || options.expandRelationshipFields) {
				// use the transformFieldValue function to get the data
				// console.log(path, transformFieldValue(field, item, options))
				data[changeCase.upperCase(changeCase.sentenceCase(path))] = transformFieldValue(field, item, options);
				return;
			}
			...
			if (field.many) {
				...
				data[changeCase.upperCase(changeCase.sentenceCase(path))] = (Array.isArray(expanded) ? expanded : []).map(function (i) {
					return i.name ? i.name + ' (' + i.id + ')' : i.id;
				}).join(', ');
			} else if (typeof expanded === 'object') {
				// for single-type relationships, add two columns to the data
				data[changeCase.upperCase(changeCase.sentenceCase(path))] = expanded.name;
				data[changeCase.upperCase(changeCase.sentenceCase(path)) + 'Id'] = expanded.id;
			}
		}, this);
	}
